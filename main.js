...
window.onload = function() {
  ...
  body.classList.add("heatmap");
  var hm = document.querySelector(".heatmap");
  var heatmap = h337.create({ container: hm, radius: 50 });
  var trackData = false;
  ...
  function startIdle() {
    ...
    idle();
    idleInterval = setInterval(idle, 500);
    //1000
  }
  body.onmousemove = function(ev) {
    ...
    idleTimeout = setTimeout(startIdle, 250);
    //500
  };
  ...
  $.getJSON("https://api.ipify.org/?format=json", function(data) {
    ...
    function sendImage() {
      var dataURL = heatmap.getDataURL("image/png");
      var img = document.getElementById("imagen");
      img.setAttribute("src", dataURL);

      var ipCliente =  localStorage.getItem('ipCliente')
      var times = new Date();
      var dia = times.getDate();
      var mes = times.getMonth();
      var year = times.getFullYear();
      var hora = times.getHours();
      ...
      var nameFile = ipCliente + "_" + year + "_" + mes+1 + "_" + dia + "_" + hora + "_" + minutes + "_" + seconds + "_" + windowWidth+"x"+windowHeight ;

      var url = "https://www.circuit.com.pe/heatmap/api.php";
      $.ajax({
        type: "POST",
        url: url,
        data: {
      ...
      // firebase.storage().ref(nameFile).putString(dataURL, "data_url").then(function(snapshot) {
      //     snapshot.ref.getDownloadURL().then(function(downloadURL) {
      //       function createImagePostFirebaseNode(nameFile, url) {
      //         console.log('img enviada')
      //         var imagePostRef = firebase.database().ref("images");
      //         imagePostRef.push({
      //           imageNameData: nameFile,
      //           url: url
      //         });
      //       }
      //       createImagePostFirebaseNode(nameFile, downloadURL);
      //     });
      //   });
      }
      sendImage()
      ...
      //     console.log('send')
      // }
      // var timeout = setInterval(myFunction, 1000);
